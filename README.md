...
- React + TypeScript + Vite 简约聊天前端
- Go HTTP 会话 API，每个浏览器会话独立保存上下文
- Telegram 机器人聊天，每个聊天独立保存上下文

## 环境要求

- Go 1.26 或更高版本
- Node.js 24 LTS（仅开发或构建 Web 前端时需要）
...
前端位于 `web`，采用 React、TypeScript 和 Vite，实现了响应式单栏聊天、新对话、连接状态、会话本地恢复和错误提示。模型 API Key 只由 Go 后端读取，不会发送到浏览器。
...
浏览器访问 `http://127.0.0.1:8080`。服务端配置如下，旧配置缺少该段时会自动使用这些默认值：
...
`allowed_origins` 只影响跨域开发请求；同源生产访问不需要加入列表。当前 Web API 没有用户登录鉴权，默认仅监听本机。若要向公网开放，请在前面配置 HTTPS、身份认证和访问限流的反向代理。
...
```text
cmd/agent                 CLI 入口
cmd/server                Web HTTP 服务入口
cmd/telegram              Telegram Bot 入口
internal/agent            对话历史与工具调用循环
internal/httpapi          会话 API、CORS 与前端静态文件服务
...
go build -o bin/roleloom-server ./cmd/server
